...
function compile_args(args, default63)
  ...
  while ____i16 < _35(____x108) do
    ...
    if has(setenv("target", {
      _stash = true,
      toplevel = true
    }), "value") == "py" and (default63 and not id_literal63(__x109)) then
      __s1 = __s1 .. "=None"
    end
    __c2 = ", "
    ____i16 = ____i16 + 1
...
local function compile_call(form)
  ...
  if method_call63(hd(__args7)) then
    __e57 = mapcat(compile, __args7, "")
  else
    __e57 = compile_args(__args7)
  end
  local __args8 = __e57
...
local function op_delims(parent, child, ...)
  ...
  if __e58(precedence(__child), precedence(__parent)) then
    return {"(", ")"}
  else
    return {"", ""}
  end
...
local function compile_infix(form)
  ...
  local ____id18 = op_delims(form, __b2, {
    _stash = true,
    right = true
  })
...
function compile_function(args, body, ...)
  ...
  if has(setenv("target", {
    ...
    if has(setenv("target", {
      ...
      __e61 = join(__args9, {"|*_args|", "|**_keys|"})
    else
      __e61 = __args9
    end
    __e60 = __e61
  end
  local __args12 = __e60
...
local function lower_if(args, hoist, stmt63, tail63)
  ...
  if stmt63 then
    ...
    if is63(___else) then
      __e73 = {lower_body({___else}, tail63)}
    end
    return add(hoist, join({"%if", lower(__cond, hoist), lower_body({___then}, tail63)}, __e73))
  else
    local __e3 = unique("e")
    add(hoist, {"%local", __e3, "nil"})
    local __e72 = nil
    if is63(___else) then
      __e72 = {lower({"%set", __e3, ___else})}
    end
    add(hoist, join({"%if", lower(__cond, hoist), lower({"%set", __e3, ___then})}, __e72))
    return __e3
  end
end
local function lower_short(x, args, hoist)
  ...
  if some63(__hoist1) then
    ...
    local __e74 = nil
    if x == "%and" then
      __e74 = {"%if", __id25, __b4, __id25}
    else
      __e74 = {"%if", __id25, __id25, __b4}
    end
    return lower({"%do", {"%local", __id25, __a4}, __e74}, hoist)
  else
...
local function lower_while(args, hoist)
  ...
  if none63(__pre1) then
    __e75 = {"%while", __c5, lower_body(__body5)}
  else
    __e75 = {"%while", true, join({"%do"}, __pre1, {{"%if", {"%not", __c5}, {"%break"}}, lower_body(__body5)})}
  end
  ...
  special = function (...)
    ...
    while ____i20 < _35(____x176) do
      ...
      if has(setenv("target", {
        _stash = true,
        toplevel = true
      }), "value") == "lua" and (immediate_call63(__x177) and "\n" == char(__s4, edge(__s4))) then
...
setenv("%try", {
  ...
  special = function (form)
    ...
    if has(setenv("target", {
      ...
      __e82 = {"%do", {"import", "sys"}, {"%local", "e", {{"%idx", "sys", "exc_info"}}}, {"%return", {"%array", false, {"%get", "e", 1}, "e"}}}
    else
      __e82 = {"%return", {"%array", false, "e"}}
    end
    local __hf1 = __e82
    setenv("indent-level", {
      _stash = true,
...
setenv("%delete", {
  ...
  special = function (place)
    ...
    return indentation() .. (__e83 .. compile(place))
  end,
  stmt = true
})
setenv("%break", {
...
setenv("%global-function", {
  ...
  special = function (name, args, ...)
    ...
    if has(setenv("target", {
      ...
      return compile({"%set", __name7, join({"%function", __args18}, __body24)}, {
        _stash = true,
        stmt = true
      })
    end
...
setenv("%local-function", {
  ...
  special = function (name, args, ...)
    ...
    if has(setenv("target", {
      ...
      return compile({"%local", __name9, join({"%function", __args20}, __body26)}, {
        _stash = true,
        stmt = true
      })
    end
...
setenv("%error", {
  ...
  special = function (x)
    ...
    if has(setenv("target", {
      ...
      __e85 = "throw " .. compile({"%new", {"Error", x}})
    else
      local __e86 = nil
      if has(setenv("target", {
        _stash = true,
        ...
        __e86 = "raise " .. compile({"Exception", x})
      else
        __e86 = "error(" .. (compile(x) .. ")")
      end
      __e85 = __e86
    end
    local __e19 = __e85
...
setenv("%object", {
  ...
  special = function (...)
    ...
    local __ind17 = ____x241
    local __e96 = nil
    if _35(__forms5) > 2 then
      __e96 = "\n" .. __ind17
    end
    local __pad1 = __e96
    local __e97 = nil
    ...
    for __k19 in next, ____o13 do
      ...
      if number63(__k19) then
        ...
        __s10 = __s10 .. (__c9 .. (key(__k20) .. (__sep3 .. ____x242)))
        __c9 = "," .. either(__pad1, " ")
      end
    end
...
setenv("%list", {
  ...
  special = function (form, comps, cond)
    ...
    local ____x244 = comps
    local ____i26 = 0
    while ____i26 < _35(____x244) do
      local ____id50 = ____x244[____i26 + 1]
...
setenv("import", {
  ...
  special = function (name, ...)
    ...
    if has(setenv("target", {
      ...
      return __ind19 .. compile({"%local", __id54, {"require", escape(__name11)}})
    end
  end,
  stmt = true
})
setenv("from", {
  ...
  special = function (name, ...)
    ...
    if hd(__imports1) == "import" then
      __e100 = tl(__imports1)
    else
      __e100 = __imports1
    end
    local __names3 = __e100
